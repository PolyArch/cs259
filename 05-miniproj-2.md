...
This is what you'll do in this project at a high level:
  ...
  0. Write a GPU kernel (e.g. convolution or attention -- hopefully already done!)
...
### Default Project: Performance Model of a GPU Kernel
...
some degree).  The default project is to model one of your kernel implementations
from mini-proj 1 (convolution or attention), running on tetracosa on the TitanV GPU.

You only need to model one kernel — pick whichever you feel you understand best or
found most interesting. If you implemented multiple versions, focus on your best one.
...
* Problem Sizes — for convolution: input channels, filter sizes, output channels; for attention: sequence length, context length, head dimension
* Tile Parameters
...
For convolution, tile parameters might determine how to split the height/width/input-channels/output-channels across GPU cores.
For attention, they might determine how queries and keys are blocked for shared memory reuse.
Those same parameters might be used to determine what data is stored in scratchpad, etc.
...
want to perform better on your implementation? To answer this question,
...
For attention, think carefully about how queries, keys, and values are blocked and what reuse that enables.
